Shader "Miao/SenrenbankaTransition"
{
    Properties
    {
        _MainTex("MainTex",2D) = "White"{}
        backgroudColor("背景颜色", Color) = (0, 0, 0, 1)
        angleX ("起始X轴角度", Range(0, 359)) = 90
        angleY ("相对Y轴角度", Range(0, 359)) = 0
        angleAxis ("主轴角度", Range(0, 359)) = 135
        [IntRange] width ("水平格子数", Range(1,20)) = 20
        [IntRange] height ("垂直格子数", Range(1,20)) = 20
        rate ("复原百分比", Range(0.1, 1)) = 0.5
        _Progress ("滑动条", Range(0, 1)) = 1

        _FadeTex("FadeTex",2D) = "White"{}
        _Fade ("淡入条", Range(0, 1)) = 0
    }
    SubShader
    {
        Tags
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                //当前条的值要算上溢出值
                float currentBarValue = minBarValue + (barValueRange + overflowValue) * _Progress;
                ...
                //采取额外对象
                float4 fadeColor = tex2D(_FadeTex, uv);
                fadeColor.a *= _Fade;
                //采样前景色
                float4 foregroundColor = tex2D(_MainTex, newUV);
                //设A覆盖B,这个公式抄的透明混合公式。
                float4 color;
                color.a =  1- (1-fadeColor.a)*(1-foregroundColor.a);
                color.rgb = fadeColor.rgb*fadeColor.a + foregroundColor.rgb * foregroundColor.a*(1-fadeColor.a) / color.a;
                ...
                color = color * (1 - isBackgroud) + backgroudColor * isBackgroud;
                //foregroundColor = foregroundColor * (1 - isBackgroud) + backgroudColor * isBackgroud;
