...
public class QQLogTimelineOdinEditorWindow : OdinEditorWindow
{
    ...
    private int messageCount
    {
        get
        {
            if (QQMessageAsset == null)
            {
                return 0;
            }
            return QQMessageAsset.messageDataList.Count;
        }
    }
    ...
    [PropertyOrder(1)]
    public List<QQTextData> messageList;
    [InfoBox("载入后，调整这根轴来选取需要处理的message数据范围")]
    [MenuItem("文本编辑器/QQLogTimelineOdin生成器",priority =100)]
    ...
    public void RefreshMessageAssetPath()
    {
        ...
        else
        {
            ...
            messageList = QQMessageAsset.messageDataList.Skip(messageListRange.x).Take(messageListRange.y - messageListRange.x).ToList();
        }
    }
    [PropertyOrder(-1)]
    ...
    {
        AssetDatabase.CreateFolder(QQLogSettings.TimeLineDirectory, QQMessageAsset.name);
        string TimeLinePath = $@"Assets\Resources\{QQLogSettings.TimeLineDirectory}\{QQMessageAsset.name}";
        timelineCreater = new QQTimelineCreater(TimeLinePath);
        for (int i = 0; i < messageList.Count; i++)
        {
            string spineAssetName = 使用默认角色名索引的SpineAsset ? messageList[i].name : 索引的SpineAsset名称;
            if (忽略生成的QQ昵称.Contains(messageList[i].name))
            {
                spineAssetName = null;
            }
            TimelineAsset timelineAsset = timelineCreater.CreateMessageTimeLine(messageList[i].SaveID, messageList[i].log, spineAssetName, $@"{messageList[i].name}\{messageList[i].SaveID}",生成轨道高级设置.SectionLength,生成轨道高级设置.ChangeMouthThreshold);
    ...
    protected override void Initialize()
    {
        ...
        {
            QQMessageAsset = settings.QQMessageAsset;
    ...
    protected void OnDisable()
    {
        ...
        settings.QQMessageAsset = QQMessageAsset;
        //}
